...
def main():
    ...
    for dataset in datasets: 
        ...
        # qimages = [os.path.dirname(__file__)[:-len('/cirtorch/examples/output/')] + '/data/']

        qimages = [os.path.dirname(__file__)[:-len(' /src/public/script/cnnimageretrieval-pytorch/examples/output/')] + '/src/public/img/image_output.jpg']
        ...
        # convert to numpy
        # vecs = vecs.numpy()        
        vecs = genfromtxt(os.path.dirname(__file__) + '/output/feature_vectors.txt', delimiter=',')
        # np.savetxt(url + '/output/feature_vectors.txt', vecs, delimiter=',') 

        qvecs = qvecs.numpy()
        np.savetxt(url + '/output/query_vectors.txt', qvecs, delimiter=',')

        # search, rank, and print
        scores = np.dot(vecs.T, qvecs)
        np.savetxt(url + '/output/scores.txt', scores, delimiter=',')
        
        ranks = np.argsort(-scores, axis=0)
        np.savetxt(url + '/output/ranks.txt', ranks, delimiter=',')
